...
const gaugeConfig = [
  { key: 'speed', label: 'Vehicle Speed', unit: 'km/h', range: [0, 120], base: 55.2, color: 'from-cyan-400 to-sky-500' },
  { key: 'rpm', label: 'Engine RPM', unit: '', range: [700, 6000], base: 2793, color: 'from-fuchsia-500 to-violet-600' },
  { key: 'coolant', label: 'Coolant Temp', unit: '°C', range: [70, 110], base: 85.8, color: 'from-amber-400 to-orange-500' },
  { key: 'maf', label: 'MAF Air Flow', unit: 'g/s', range: [2, 25], base: 9.4, color: 'from-emerald-400 to-lime-500' },
  { key: 'fuel', label: 'Fuel Level', unit: '%', range: [0, 100], base: 40.1, color: 'from-rose-400 to-red-500' },
];
...
export default function TelemetryPanel() {
  const [telemetry, setTelemetry] = useState(defaultData);

  useEffect(() => {
    const interval = setInterval(() => {
      setTelemetry((prev) => ({
        speed: clamp(prev.speed + (Math.random() - 0.5) * 8, 0, 120),
        rpm: clamp(prev.rpm + (Math.random() - 0.5) * 140, 700, 6000),
        coolant: clamp(prev.coolant + (Math.random() - 0.5) * 1.6, 70, 110),
        maf: clamp(prev.maf + (Math.random() - 0.5) * 0.75, 2, 25),
        fuel: clamp(prev.fuel - (Math.random() > 0.92 ? 0.3 : 0.03), 0, 100),
      }));
    }, 220);
  ...
  const gauges = useMemo(() =>
    gaugeConfig.map((gauge) => {
      const value = telemetry[gauge.key];
      const [min, max] = gauge.range;
      const normalized = (value - min) / (max - min);
      const percent = clamp(Math.round(normalized * 100), 0, 100);
      const isAlert = gauge.key === 'coolant' ? value > 105 : gauge.key === 'fuel' ? value < 15 : false;
        ...
        {gauges.map((gauge) => (
          <div key={gauge.key} className="rounded-2xl border border-slate-800 bg-slate-950/70 p-4">
            <div className="mb-3 flex items-center justify-between">
              ...
              <span className={`rounded-full px-2 py-1 text-[10px] uppercase tracking-[0.25em] ${gauge.isAlert ? 'bg-red-500/20 text-red-300' : 'bg-slate-800 text-slate-400'}`}>
            ...
            <div className="relative h-32 overflow-hidden rounded-2xl bg-slate-900/70">
              ...
              <div className="absolute inset-0 flex flex-col justify-end p-4">
                <div className="text-3xl font-semibold text-white">
                  {gauge.key === 'rpm' ? Math.round(gauge.value).toLocaleString() : gauge.value.toFixed(gauge.key === 'speed' || gauge.key === 'maf' || gauge.key === 'fuel' ? 1 : 0)}
                  <span className="ml-1 text-sm text-slate-400">{gauge.unit}</span>
                </div>
